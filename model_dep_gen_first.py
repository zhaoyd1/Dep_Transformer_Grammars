...
def forward_prepare(model, pad_id, left_arc, right_arc, pop_root, ranges, x, startofword_x, length, use_mask=None, document_level=False, return_h=False, max_relative_length=None, min_relative_length=None):
    attn_mask = []
    attn_relpos = []
    inputs = []
    targets = []
    ...
    if use_mask is None:
        ...
        for sent in x:
            src_ = sent[:-1]
            tgt_ = sent[1:]
            src_p = src_ + [pad_id] * (length_i - len(src_))
            inputs.append(np.array(src_p))
            tgt_p = tgt_ + [pad_id] * (length_i - len(tgt_))
            ...
            if not document_level:
                ...
                if use_mask != 'txl_arc':
                    for i in range(len(sent)):
                        if sent[i] == left_arc or sent[i] == right_arc:
        ...
        for sent, sent_startofword in zip(x, startofword_x):
            ...
            if not document_level:
                ...
                # print(src_p)
                composed_pos = chunk.composed_position[:length-1]
                if use_mask == 'sub+action' or use_mask == 'sub+action_1':
                    original_action.append(np.array(src_p))
                    action_indicator.append((src_p == left_arc) | (src_p == right_arc) | (src_p == pop_root))
                #     print(src_p)
                ...
                if use_mask != 'sub+action_1':
                    tgt_action = [0 if action == pad_id else 1 if action == left_arc else 2 if action == right_arc else 3 for action in tgt_p]
                    targets_action.append(np.array(tgt_action))
                    tgt_p = [pad_id if action == left_arc or action == right_arc else action for action in tgt_p]
                # new_length, = np.where(tgt_p == self.eos_id)
                ...
                # exit()
                # if use_mask == 'first0' or use_mask == 'sub+action' or use_mask == 'sub+action_1':
                #     relpos[:, 0] = 0
                attn_relpos.append(np.array(relpos))
            else:
                pass #remain to be implemented
        if len(original_action) > 0:
            original_action = torch.LongTensor(np.array(original_action)).cuda()
            action_indicator = torch.BoolTensor(np.array(action_indicator)).cuda()
        inputs = torch.LongTensor(np.array(inputs)).cuda()
        targets = torch.LongTensor(np.array(targets)).cuda()
        targets_action = torch.LongTensor(np.array(targets_action)).cuda()
        attn_mask = torch.LongTensor(np.array(attn_mask)).cuda().bool()
        attn_relpos = torch.LongTensor(np.array(attn_relpos)).cuda()

    return model(original_action, action_indicator, inputs, targets, targets_action, attn_mask, attn_relpos, use_mask, document_level, return_h, 
                        max_relative_length, min_relative_length)
...
class TransformerGrammar(nn.Module):
    ...
    def forward(self, original_action, action_indicator, inputs, targets, targets_action, attn_mask, attn_relpos, use_mask=None, document_level=False, return_h=False, max_relative_length=None, min_relative_length=None):
        ...
        #             if use_mask == 'sub+action' or use_mask == 'sub+action_1':
        #                 original_action.append(np.array(src_p))
        #                 action_indicator.append((src_p == self.left_arc) | (src_p == self.right_arc) | (src_p == self.pop_root))
        #             #     print(src_p)
        #             #     print(action_indicator)
        #             # exit()
        #             if use_mask != 'arc':
        ...
        #             tgt_p = chunk.labels[:length-1]
        #             if use_mask != 'sub+action_1':
        #                 tgt_action = [0 if action == self.pad_id else 1 if action == self.left_arc else 2 if action == self.right_arc else 3 for action in tgt_p]
        #                 targets_action.append(np.array(tgt_action))
        #                 tgt_p = [self.pad_id if action == self.left_arc or action == self.right_arc else action for action in tgt_p]
        #             # new_length, = np.where(tgt_p == self.eos_id)
        #             # new_length = new_length[0]
        #             # new_length += 1
        #             # print(tgt_p)
        ...
        if use_mask == 'linear':
            attn_relpos = None
        if len(original_action) > 0:
            original_action = original_action.permute(1, 0).contiguous()
            action_indicator = action_indicator.permute(1, 0).contiguous()
        inputs = inputs.permute(1, 0).contiguous()
        targets = targets.permute(1, 0).contiguous()
        if use_mask != 'sub+action_1':
            targets_action = targets_action.permute(1, 0).contiguous()
        ...
        if use_mask == 'sub+action' or use_mask == 'sub+action_1':
            word_emb += self.emb(original_action) * action_indicator.unsqueeze(-1)
        ...
        if use_mask != 'sub+action_1':
            actions = self.action_mlp(core_out)
            action_prob = actions.view(seq_len, batch, -1)
            action_prob = action_prob.permute(0, 2, 1)
            crit_action = nn.CrossEntropyLoss(reduction='none', ignore_index=0)
            loss_action = crit_action(action_prob, targets_action)
            loss_action = loss_action.permute(1, 0).contiguous()
            loss_action = loss_action.sum(1)
    ...
    def constrained_forward_gen(self, 
        ...
        with torch.no_grad():
            ...
            if not use_mask.startswith('txl') and not use_mask.startswith('linear'):
            ...
            actions = self.action_mlp(core_out)
            #add constraints!!!
            action_prob = actions.view(1, batch, -1) # 1 * batch * 4
            action_prob = action_prob.log_softmax(-1)
            word_prob = self.projection(core_out) # 1 * batch * vocab
            word_prob = word_prob.log_softmax(-1)
            word_prob = action_prob[:, :, 3].unsqueeze(-1) + word_prob
            word_prob[:, :, self.left_arc] = action_prob[:, :, 1]
            word_prob[:, :, self.right_arc] = action_prob[:, :, 2]
            # print(action_prob)
            prob = word_prob.view(1, batch, -1)
